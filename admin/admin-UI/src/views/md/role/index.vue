    ...
    <div class="card-container search-card">
      <el-card shadow="never">
        <div slot="header" class="card-header">
          <span>{{ $t('domain.role.pageTitle') }}</span>
        </div>
        <el-row :gutter="10" align="middle">
          ...
          <el-col :md="8" :sm="12" :xs="24" class="grid-cell">
            <el-form-item :label="'lastUpdated'|i18n('role')">
              <el-input v-model="tableQuery.search.lastUpdated" type="text" clearable />
            </el-form-item>
          </el-col>
    ...
    <div class="card-container list-card">
      <el-card style="{width: 100% !important}" shadow="never">
        ...
        >
          ...
          <el-table-column v-if="config.props.name.visible" :label="'name'|i18n('role')" prop="name" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type" @click="handleShow(row.id)">{{ row.name }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.code.visible" :label="'code'|i18n('role')" prop="code" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type" @click="handleShow(row.id)">{{ row.code }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.fullName.visible" :label="'fullName'|i18n('role')" prop="fullName" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.fullName }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.dateCreated.visible" :label="'dateCreated'|i18n('role')" prop="dateCreated" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.dateCreated }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.lastUpdated.visible" :label="'lastUpdated'|i18n('role')" prop="lastUpdated" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.lastUpdated }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.org.visible" :label="'org'|i18n('role')" prop="org" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type span-inline" @click="handleShowChildren(row.org.id, 'org', 'Org')">{{ row.org }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.roleGroup.visible" :label="'roleGroup'|i18n('role')" prop="roleGroup" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type span-inline" @click="handleShowChildren(row.roleGroup.id, 'roleGroup', 'RoleGroup')">{{ row.roleGroup }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.roleUsers.visible" :label="'roleUsers'|i18n('role')" prop="roleUsers" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
          ...
          <el-table-column v-if="config.props.id.visible" :label="'id'|i18n('role')" prop="id" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.id }}</span>
            </template>
          </el-table-column>
...
import RoleGroupShowView from '@/views/md/roleGroup/show'
import RoleGroupEditView from '@/views/md/roleGroup/edit'

export default {
  name: 'RoleList', // name值需要与router中的name相同，否则页签的“keep-alive”功能不生效

  components: { Pagination, ShowView, EditView, RoleUserShowView, OrgShowView, RoleGroupShowView, RoleUserEditView, OrgEditView, RoleGroupEditView },
  data() {
    return {
      ...baseIndex.data,
      config: {
        ...
        props: {
          ...
          name: {
            title: i18n('name', 'role'),
            defVisible: true,
            visible: true
          },

          code: {
            title: i18n('code', 'role'),
            defVisible: true,
            visible: true
          },

          fullName: {
            title: i18n('fullName', 'role'),
            defVisible: true,
            visible: true
          },

          dateCreated: {
            title: i18n('dateCreated', 'role'),
            defVisible: true,
            visible: true
          },

          lastUpdated: {
            title: i18n('lastUpdated', 'role'),
            defVisible: true,
            visible: true
          },
          ...
          roleGroup: {
            title: i18n('roleGroup', 'role'),
            defVisible: false
          },

          roleUsers: {
            title: i18n('roleUsers', 'role'),
            defVisible: false
          },
        ...
        // 导出Excel配置
        download: {
          excelName: this.$t('domain.role.pageTitle'),
