...
<template>
  <el-dialog :title="$t('domain.def.select')+'——'+$t('domain.role.pageTitle')" :visible.sync="config.seVisible" :append-to-body="config.appendToBody" width="80%" class="show-dialog" @open="openDialog">
      ...
      <div class="card-container select-card">
        <el-card shadow="always">
          ...
          >
            ...
            <el-table-column v-if="columnVisible('name')" :label="'name'|i18n('role')" prop="name" sortable="custom" min-width="100px" align="center">
              <template slot-scope="{row}">
                <span class="link-type" @click="handleShow(row.id)">{{ row.name }}</span>
              </template>
            </el-table-column>

            <el-table-column v-if="columnVisible('code')" :label="'code'|i18n('role')" prop="code" sortable="custom" min-width="100px" align="center">
              <template slot-scope="{row}">
                <span class="link-type" @click="handleShow(row.id)">{{ row.code }}</span>
              </template>
            </el-table-column>

            <el-table-column v-if="columnVisible('fullName')" :label="'fullName'|i18n('role')" prop="fullName" sortable="custom" min-width="100px" align="center">
              <template slot-scope="{row}">
                <span>{{ row.fullName }}</span>
              </template>
            </el-table-column>

            <el-table-column v-if="columnVisible('dateCreated')" :label="'dateCreated'|i18n('role')" prop="dateCreated" sortable="custom" min-width="100px" align="center">
              <template slot-scope="{row}">
                <span>{{ row.dateCreated }}</span>
              </template>
            </el-table-column>

            <el-table-column v-if="columnVisible('lastUpdated')" :label="'lastUpdated'|i18n('role')" prop="lastUpdated" sortable="custom" min-width="100px" align="center">
              <template slot-scope="{row}">
                <span>{{ row.lastUpdated }}</span>
              </template>
            </el-table-column>

            <el-table-column v-if="columnVisible('org')" :label="'org'|i18n('role')" prop="org" sortable="custom" min-width="100px" align="center">
              <template slot-scope="{row}">
                <span class="link-type span-inline" @click="handleShowChildren(row.org.id, 'org', 'Org')">{{ row.org }}</span>
              </template>
            </el-table-column>

            <el-table-column v-if="columnVisible('roleGroup')" :label="'roleGroup'|i18n('role')" prop="roleGroup" sortable="custom" min-width="100px" align="center">
              <template slot-scope="{row}">
                <span class="link-type span-inline" @click="handleShowChildren(row.roleGroup.id, 'roleGroup', 'RoleGroup')">{{ row.roleGroup }}</span>
              </template>
            </el-table-column>

            <el-table-column v-if="columnVisible('roleUsers')" :label="'roleUsers'|i18n('role')" prop="roleUsers" sortable="custom" min-width="100px" align="center">
              <template slot-scope="{row}">
            ...
            <el-table-column v-if="columnVisible('id')" :label="'id'|i18n('role')" prop="id" sortable="custom" min-width="100px" align="center">
              <template slot-scope="{row}">
                <span>{{ row.id }}</span>
              </template>
            </el-table-column>
