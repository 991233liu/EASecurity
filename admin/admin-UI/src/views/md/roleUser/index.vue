    ...
    <div class="card-container search-card">
      <el-card shadow="never">
        <div slot="header" class="card-header">
          <span>{{ $t('domain.roleUser.pageTitle') }}</span>
        </div>
        <el-row :gutter="10" align="middle">
          ...
          <el-col :md="8" :sm="12" :xs="24" class="grid-cell">
            <el-form-item :label="'roleCode'|i18n('roleUser')">
              <el-input v-model="tableQuery.search.roleCode" type="text" clearable />
            </el-form-item>
          </el-col>
          ...
          <el-col :md="8" :sm="12" :xs="24" class="grid-cell">
            <el-form-item :label="'role'|i18n('roleUser')">
              <el-input v-model="tableQuery.search.role" type="text" clearable />
            </el-form-item>
          </el-col>
    ...
    <div class="card-container list-card">
      <el-card style="{width: 100% !important}" shadow="never">
        ...
        >
          ...
          <el-table-column v-if="config.props.account.visible" :label="'account'|i18n('roleUser')" prop="account" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type" @click="handleShow(row.id)">{{ row.account }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.roleCode.visible" :label="'roleCode'|i18n('roleUser')" prop="roleCode" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type" @click="handleShow(row.id)">{{ row.roleCode }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.dateCreated.visible" :label="'dateCreated'|i18n('roleUser')" prop="dateCreated" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.dateCreated }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.lastUpdated.visible" :label="'lastUpdated'|i18n('roleUser')" prop="lastUpdated" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.lastUpdated }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.role.visible" :label="'role'|i18n('roleUser')" prop="role" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type span-inline" @click="handleShowChildren(row.role.id, 'role', 'Role')">{{ row.role }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.user.visible" :label="'user'|i18n('roleUser')" prop="user" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type span-inline" @click="handleShowChildren(row.user.id, 'user', 'User')">{{ row.user }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.id.visible" :label="'id'|i18n('roleUser')" prop="id" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.id }}</span>
            </template>
          </el-table-column>
...
import RoleShowView from '@/views/md/role/show'
import RoleEditView from '@/views/md/role/edit'

import UserShowView from '@/views/md/user/show'
import UserEditView from '@/views/md/user/edit'

export default {
  name: 'RoleUserList', // name值需要与router中的name相同，否则页签的“keep-alive”功能不生效

  components: { Pagination, ShowView, EditView, RoleShowView, UserShowView, RoleEditView, UserEditView },
  data() {
    return {
      ...baseIndex.data,
      config: {
        ...
        props: {
          ...
          account: {
            title: i18n('account', 'roleUser'),
            defVisible: true,
            visible: true
          },

          roleCode: {
            title: i18n('roleCode', 'roleUser'),
            defVisible: true,
            visible: true
          },

          dateCreated: {
            title: i18n('dateCreated', 'roleUser'),
            defVisible: true,
            visible: true
          },

          lastUpdated: {
            title: i18n('lastUpdated', 'roleUser'),
            defVisible: true,
            visible: true
          },

          role: {
            title: i18n('role', 'roleUser'),
            defVisible: true,
            visible: true
          },

          user: {
            title: i18n('user', 'roleUser'),
            defVisible: true,
            visible: true
          },

          id: {
            title: i18n('id', 'roleUser'),
            defVisible: false
          }
        ...
        // 导出Excel配置
        download: {
          excelName: this.$t('domain.roleUser.pageTitle'),
