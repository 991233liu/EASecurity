    ...
    <div class="card-container search-card">
      <el-card shadow="never">
        <div slot="header" class="card-header">
          <span>{{ $t('domain.roleGroup.pageTitle') }}</span>
        </div>
        <el-row :gutter="10" align="middle">
          ...
          <el-col :md="8" :sm="12" :xs="24" class="grid-cell">
            <el-form-item :label="'roles'|i18n('roleGroup')">
              <el-input v-model="tableQuery.search.roles" type="text" clearable />
            </el-form-item>
          </el-col>
    ...
    <div class="card-container list-card">
      <el-card style="{width: 100% !important}" shadow="never">
        ...
        >
          ...
          <el-table-column v-if="config.props.name.visible" :label="'name'|i18n('roleGroup')" prop="name" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type" @click="handleShow(row.id)">{{ row.name }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.code.visible" :label="'code'|i18n('roleGroup')" prop="code" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type" @click="handleShow(row.id)">{{ row.code }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.dateCreated.visible" :label="'dateCreated'|i18n('roleGroup')" prop="dateCreated" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.dateCreated }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.lastUpdated.visible" :label="'lastUpdated'|i18n('roleGroup')" prop="lastUpdated" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.lastUpdated }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.roles.visible" :label="'roles'|i18n('roleGroup')" prop="roles" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span v-for="(item, index) in row.roles" :key="index" class="link-type span-inline" @click="handleShowChildren(item.id, 'roles', 'Role')">{{ item }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.id.visible" :label="'id'|i18n('roleGroup')" prop="id" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.id }}</span>
            </template>
          </el-table-column>
...
export default {
  name: 'RoleGroupList', // name值需要与router中的name相同，否则页签的“keep-alive”功能不生效

  components: { Pagination, ShowView, EditView, RoleShowView, RoleEditView },
  data() {
    return {
      ...baseIndex.data,
      config: {
        ...
        props: {
          ...
          name: {
            title: i18n('name', 'roleGroup'),
            defVisible: true,
            visible: true
          },

          code: {
            title: i18n('code', 'roleGroup'),
            defVisible: true,
            visible: true
          },

          dateCreated: {
            title: i18n('dateCreated', 'roleGroup'),
            defVisible: true,
            visible: true
          },

          lastUpdated: {
            title: i18n('lastUpdated', 'roleGroup'),
            defVisible: true,
            visible: true
          },

          roles: {
            title: i18n('roles', 'roleGroup'),
            defVisible: true,
            visible: true
          },

          id: {
            title: i18n('id', 'roleGroup'),
            defVisible: true,
            visible: true
          }
        ...
        // 导出Excel配置
        download: {
          excelName: this.$t('domain.roleGroup.pageTitle'),
