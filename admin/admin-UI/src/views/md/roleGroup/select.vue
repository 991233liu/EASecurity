...
<template>
  <el-dialog :title="$t('domain.def.select')+'——'+$t('domain.roleGroup.pageTitle')" :visible.sync="config.seVisible" :append-to-body="config.appendToBody" width="80%" class="show-dialog" @open="openDialog">
      ...
      <div class="card-container select-card">
        <el-card shadow="always">
          ...
          >
            ...
            <el-table-column v-if="columnVisible('name')" :label="'name'|i18n('roleGroup')" prop="name" sortable="custom" min-width="100px" align="center">
              <template slot-scope="{row}">
                <span class="link-type" @click="handleShow(row.id)">{{ row.name }}</span>
              </template>
            </el-table-column>

            <el-table-column v-if="columnVisible('code')" :label="'code'|i18n('roleGroup')" prop="code" sortable="custom" min-width="100px" align="center">
              <template slot-scope="{row}">
                <span class="link-type" @click="handleShow(row.id)">{{ row.code }}</span>
              </template>
            </el-table-column>

            <el-table-column v-if="columnVisible('dateCreated')" :label="'dateCreated'|i18n('roleGroup')" prop="dateCreated" sortable="custom" min-width="100px" align="center">
              <template slot-scope="{row}">
                <span>{{ row.dateCreated }}</span>
              </template>
            </el-table-column>

            <el-table-column v-if="columnVisible('lastUpdated')" :label="'lastUpdated'|i18n('roleGroup')" prop="lastUpdated" sortable="custom" min-width="100px" align="center">
              <template slot-scope="{row}">
                <span>{{ row.lastUpdated }}</span>
              </template>
            </el-table-column>

            <el-table-column v-if="columnVisible('roles')" :label="'roles'|i18n('roleGroup')" prop="roles" sortable="custom" min-width="100px" align="center">
              <template slot-scope="{row}">
            ...
            <el-table-column v-if="columnVisible('id')" :label="'id'|i18n('roleGroup')" prop="id" sortable="custom" min-width="100px" align="center">
              <template slot-scope="{row}">
                <span>{{ row.id }}</span>
              </template>
            </el-table-column>
