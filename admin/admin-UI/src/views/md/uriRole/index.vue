    ...
    <div class="card-container search-card">
      <el-card shadow="never">
        <div slot="header" class="card-header">
          <span>{{ $t('domain.uriRole.pageTitle') }}</span>
        </div>
        <el-row :gutter="10" align="middle">

          <el-col :md="8" :sm="12" :xs="24" class="grid-cell">
            <el-form-item :label="'roleId'|i18n('uriRole')">
              <el-input v-model="tableQuery.search.roleId" type="text" clearable />
            </el-form-item>
          </el-col>

          <el-col :md="8" :sm="12" :xs="24" class="grid-cell">
            <el-form-item :label="'annotation'|i18n('uriRole')">
              <el-input v-model="tableQuery.search.annotation" type="text" clearable />
            </el-form-item>
          </el-col>

          <el-col :md="8" :sm="12" :xs="24" class="grid-cell">
            <el-form-item :label="'group1'|i18n('uriRole')">
              <el-input v-model="tableQuery.search.group1" type="text" clearable />
            </el-form-item>
          </el-col>

          <el-col :md="8" :sm="12" :xs="24" class="grid-cell">
            <el-form-item :label="'fromTo'|i18n('uriRole')">
              <el-input v-model="tableQuery.search.fromTo" type="text" clearable />
            </el-form-item>
          </el-col>

          <el-col :md="8" :sm="12" :xs="24" class="grid-cell">
            <el-form-item :label="'status'|i18n('uriRole')">
    ...
    <div class="card-container list-card">
      <el-card style="{width: 100% !important}" shadow="never">
        ...
        >
          ...
          <el-table-column v-if="config.props.roleId.visible" :label="'roleId'|i18n('uriRole')" prop="roleId" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type" @click="handleShow(row.id)">{{ row.roleId }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.annotation.visible" :label="'annotation'|i18n('uriRole')" prop="annotation" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type" @click="handleShow(row.id)">{{ row.annotation }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.group1.visible" :label="'group1'|i18n('uriRole')" prop="group1" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.group1 }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.fromTo.visible" :label="'fromTo'|i18n('uriRole')" prop="fromTo" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.fromTo | fmatEnum(enumClasses['com.easecurity.core.basis.au.UriRoleEnum$FromTo']) }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.status.visible" :label="'status'|i18n('uriRole')" prop="status" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.status | fmatEnum(enumClasses['com.easecurity.core.basis.au.UriRoleEnum$Status']) }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.uri.visible" :label="'uri'|i18n('uriRole')" prop="uri" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type span-inline" @click="handleShowChildren(row.uri.id, 'uri', 'Uri')">{{ row.uri }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.id.visible" :label="'id'|i18n('uriRole')" prop="id" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.id }}</span>
            </template>
          </el-table-column>
...
export default {
  name: 'UriRoleList', // name值需要与router中的name相同，否则页签的“keep-alive”功能不生效
    ...
    return {
      ...
      config: {
        ...
        props: {
          ...
          roleId: {
            title: i18n('roleId', 'uriRole'),
            defVisible: true,
            visible: true
          },

          annotation: {
            title: i18n('annotation', 'uriRole'),
            defVisible: true,
            visible: true
          },

          group1: {
            title: i18n('group1', 'uriRole'),
            defVisible: true,
            visible: true
          },

          fromTo: {
            title: i18n('fromTo', 'uriRole'),
            defVisible: true,
            visible: true
          },

          status: {
            title: i18n('status', 'uriRole'),
            defVisible: true,
            visible: true
          },

          uri: {
            title: i18n('uri', 'uriRole'),
            defVisible: true,
            visible: true
          },

          id: {
            title: i18n('id', 'uriRole'),
            defVisible: false
          }
        ...
        // 导出Excel配置
        download: {
          excelName: this.$t('domain.uriRole.pageTitle'),
      ...
      enumClasses: {
        ...
        'com.easecurity.core.basis.au.UriRoleEnum$FromTo': this.getEnum('com.easecurity.core.basis.au.UriRoleEnum$FromTo'),

        'com.easecurity.core.basis.au.UriRoleEnum$Status': this.getEnum('com.easecurity.core.basis.au.UriRoleEnum$Status')
