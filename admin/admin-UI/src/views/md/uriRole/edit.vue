...
<template>
  <el-dialog :title="$t('domain.def.' + config.eStatus)+'——'+$t('domain.uriRole.pageTitle')" :visible.sync="config.eVisible" :append-to-body="config.appendToBody" width="80%" class="edit-dialog" :close-on-click-modal="false" @open="openDialog">
        ...
        <el-card shadow="always">
          <el-row :gutter="10" align="middle" class="edit-row">
            ...
            <el-col :md="12" :sm="12" :xs="24" min-width="400px" class="grid-cell">
              <el-form-item :label="'roleId'|i18n('uriRole')" prop="roleId">
                <el-input id="roleId" v-model="temp.roleId" clearable />
              </el-form-item>
            </el-col>

            <el-col :md="12" :sm="12" :xs="24" min-width="400px" class="grid-cell">
              <el-form-item :label="'annotation'|i18n('uriRole')" prop="annotation">
                <el-input id="annotation" v-model="temp.annotation" clearable />
              </el-form-item>
            </el-col>

            <el-col :md="12" :sm="12" :xs="24" min-width="400px" class="grid-cell">
              <el-form-item :label="'group1'|i18n('uriRole')" prop="group1">
                <el-input id="group1" v-model="temp.group1" clearable />
              </el-form-item>
            </el-col>

            <el-col :md="12" :sm="12" :xs="24" min-width="400px" class="grid-cell">
              <el-form-item :label="'fromTo'|i18n('uriRole')" prop="fromTo">
                <el-select id="fromTo" v-model="temp.fromTo" :placeholder="$t('domain.def.selectPlaceholder')"><el-option v-for="(item, index) in enumClasses['com.easecurity.core.basis.au.UriRoleEnum$FromTo']" :key="index" :label="item.title" :value="item.code" /></el-select>
              </el-form-item>
            </el-col>

            <el-col :md="12" :sm="12" :xs="24" min-width="400px" class="grid-cell">
              <el-form-item :label="'status'|i18n('uriRole')" prop="status">
                <el-select id="status" v-model="temp.status" :placeholder="$t('domain.def.selectPlaceholder')"><el-option v-for="(item, index) in enumClasses['com.easecurity.core.basis.au.UriRoleEnum$Status']" :key="index" :label="item.title" :value="item.code" /></el-select>
              </el-form-item>
            </el-col>

            <el-col :md="12" :sm="12" :xs="24" min-width="400px" class="grid-cell">
              <el-form-item :label="'uri'|i18n('uriRole')" prop="uri">
                <div class="span-inline"><span class="link-type" @click="handleShowChildren(temp.uri.id, 'uri', 'Uri')">{{ temp.uri }}</span><el-link type="danger" icon="el-icon-delete" @click="handleDeleteChildren(temp.uri.id, 'uri', 'Uri')">{{ $t('domain.def.delete') }}</el-link></div><el-link type="primary" icon="el-icon-edit" @click="handleCreateChildren('uri', 'Uri', 'uriRole', temp.id)">{{ $t('domain.def.add') }}</el-link>
              </el-form-item>
            </el-col>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        roleId: [{ type: 'integer', message: 'roleId must be integer', trigger: 'change' }],
