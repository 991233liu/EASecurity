...
<template>
  <el-dialog :title="$t('domain.def.select')+'——'+$t('domain.userAdmin.pageTitle')" :visible.sync="config.seVisible" :append-to-body="config.appendToBody" width="80%" class="show-dialog" @open="openDialog">
      ...
      <div class="card-container select-card">
        <el-card shadow="always">
          ...
          >
            ...
            <el-table-column v-if="columnVisible('account')" :label="'account'|i18n('userAdmin')" prop="account" sortable="custom" min-width="100px" align="center">
              <template slot-scope="{row}">
                <span class="link-type" @click="handleShow(row.id)">{{ row.account }}</span>
              </template>
            </el-table-column>

            <el-table-column v-if="columnVisible('id')" :label="'id'|i18n('userAdmin')" prop="id" sortable="custom" min-width="100px" align="center">
              <template slot-scope="{row}">
