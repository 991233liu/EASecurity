    ...
    <div class="card-container search-card">
      <el-card shadow="never">
        <div slot="header" class="card-header">
          <span>{{ $t('domain.userAdmin.pageTitle') }}</span>
        </div>
        <el-row :gutter="10" align="middle">
          ...
          <el-col :md="8" :sm="12" :xs="24" class="grid-cell">
            <el-form-item :label="'id'|i18n('userAdmin')">
              <el-input v-model="tableQuery.search.id" type="text" clearable />
            </el-form-item>
          </el-col>
    ...
    <div class="card-container list-card">
      <el-card style="{width: 100% !important}" shadow="never">
        ...
        >
          ...
          <el-table-column v-if="config.props.account.visible" :label="'account'|i18n('userAdmin')" prop="account" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type" @click="handleShow(row.id)">{{ row.account }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.id.visible" :label="'id'|i18n('userAdmin')" prop="id" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type" @click="handleShow(row.id)">{{ row.id }}</span>
            </template>
          </el-table-column>
...
export default {
  name: 'UserAdminList', // name值需要与router中的name相同，否则页签的“keep-alive”功能不生效
  ...
  data() {
    return {
      ...
      config: {
        id: undefined,
        domainName: 'userAdmin',
        // 属性列表
        props: {

          account: {
            title: i18n('account', 'userAdmin'),
            defVisible: true,
            visible: true
          },

          id: {
            title: i18n('id', 'userAdmin'),
            defVisible: true,
            visible: true
          }
        ...
        // 导出Excel配置
        download: {
          excelName: this.$t('domain.userAdmin.pageTitle'),
