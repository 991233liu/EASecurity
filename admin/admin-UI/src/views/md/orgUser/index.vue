    ...
    <div class="card-container search-card">
      <el-card shadow="never">
        <div slot="header" class="card-header">
          <span>{{ $t('domain.orgUser.pageTitle') }}</span>
        </div>
        <el-row :gutter="10" align="middle">
          ...
          <el-col :md="8" :sm="12" :xs="24" class="grid-cell">
            <el-form-item :label="'org'|i18n('orgUser')">
              <el-input v-model="tableQuery.search.org" type="text" clearable />
            </el-form-item>
          </el-col>
    ...
    <div class="card-container list-card">
      <el-card style="{width: 100% !important}" shadow="never">
        ...
        >
          ...
          <el-table-column v-if="config.props.account.visible" :label="'account'|i18n('orgUser')" prop="account" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type" @click="handleShow(row.id)">{{ row.account }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.masterPosts.visible" :label="'masterPosts'|i18n('orgUser')" prop="masterPosts" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type" @click="handleShow(row.id)">{{ row.masterPosts | fmatBoolean }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.dateCreated.visible" :label="'dateCreated'|i18n('orgUser')" prop="dateCreated" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.dateCreated }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.lastUpdated.visible" :label="'lastUpdated'|i18n('orgUser')" prop="lastUpdated" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.lastUpdated }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.org.visible" :label="'org'|i18n('orgUser')" prop="org" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type span-inline" @click="handleShowChildren(row.org.id, 'org', 'Org')">{{ row.org }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.posts.visible" :label="'posts'|i18n('orgUser')" prop="posts" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type span-inline" @click="handleShowChildren(row.posts.id, 'posts', 'Posts')">{{ row.posts }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.user.visible" :label="'user'|i18n('orgUser')" prop="user" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type span-inline" @click="handleShowChildren(row.user.id, 'user', 'User')">{{ row.user }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.id.visible" :label="'id'|i18n('orgUser')" prop="id" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.id }}</span>
            </template>
          </el-table-column>
...
import PostsShowView from '@/views/md/posts/show'
import PostsEditView from '@/views/md/posts/edit'

import UserShowView from '@/views/md/user/show'
import UserEditView from '@/views/md/user/edit'

export default {
  name: 'OrgUserList', // name值需要与router中的name相同，否则页签的“keep-alive”功能不生效

  components: { Pagination, ShowView, EditView, OrgShowView, PostsShowView, UserShowView, OrgEditView, PostsEditView, UserEditView },
  data() {
    return {
      ...baseIndex.data,
      config: {
        ...
        props: {
          ...
          account: {
            title: i18n('account', 'orgUser'),
            defVisible: true,
            visible: true
          },

          masterPosts: {
            title: i18n('masterPosts', 'orgUser'),
            defVisible: true,
            visible: true
          },

          dateCreated: {
            title: i18n('dateCreated', 'orgUser'),
            defVisible: true,
            visible: true
          },

          lastUpdated: {
            title: i18n('lastUpdated', 'orgUser'),
            defVisible: true,
            visible: true
          },

          org: {
            title: i18n('org', 'orgUser'),
            defVisible: true,
            visible: true
          },

          posts: {
            title: i18n('posts', 'orgUser'),
            defVisible: true,
            visible: true
          },

          user: {
            title: i18n('user', 'orgUser'),
            defVisible: false
          },

          id: {
            title: i18n('id', 'orgUser'),
            defVisible: false
          }
        ...
        // 导出Excel配置
        download: {
          excelName: this.$t('domain.orgUser.pageTitle'),
