...
<template>
  <el-dialog :title="$t('domain.def.' + config.eStatus)+'——'+$t('domain.posts.pageTitle')" :visible.sync="config.eVisible" :append-to-body="config.appendToBody" width="80%" class="edit-dialog" :close-on-click-modal="false" @open="openDialog">
        ...
        <el-card shadow="always">
          <el-row :gutter="10" align="middle" class="edit-row">
            ...
            <el-col :md="12" :sm="12" :xs="24" min-width="400px" class="grid-cell">
              <el-form-item :label="'name'|i18n('posts')" prop="name">
                <el-input id="name" v-model="temp.name" clearable />
              </el-form-item>
            </el-col>

            <el-col :md="12" :sm="12" :xs="24" min-width="400px" class="grid-cell">
              <el-form-item :label="'code'|i18n('posts')" prop="code">
                <el-input id="code" v-model="temp.code" clearable />
              </el-form-item>
            </el-col>

            <el-col :md="12" :sm="12" :xs="24" min-width="400px" class="grid-cell">
              <el-form-item :label="'ranking'|i18n('posts')" prop="ranking">
                <el-input id="ranking" v-model="temp.ranking" clearable />
              </el-form-item>
            </el-col>

            <el-col :md="12" :sm="12" :xs="24" min-width="400px" class="grid-cell">
              <el-form-item :label="'type'|i18n('posts')" prop="type">
                <el-select id="type" v-model="temp.type" :placeholder="$t('domain.def.selectPlaceholder')"><el-option v-for="(item, index) in enumClasses['com.easecurity.core.basis.b.PostsEnum$Type']" :key="index" :label="item.title" :value="item.code" /></el-select>
              </el-form-item>
            </el-col>

            <el-col :md="12" :sm="12" :xs="24" min-width="400px" class="grid-cell">
              <el-form-item :label="'dateCreated'|i18n('posts')" prop="dateCreated">
            ...
            <el-col :md="12" :sm="12" :xs="24" min-width="400px" class="grid-cell">
              <el-form-item :label="'lastUpdated'|i18n('posts')" prop="lastUpdated">
            ...
            <el-col :md="12" :sm="12" :xs="24" min-width="400px" class="grid-cell">
              <el-form-item :label="'orgUsers'|i18n('posts')" prop="orgUsers">
                <div v-for="(item, index) in temp.orgUsers" :key="index" class="span-inline"><span class="link-type" @click="handleShowChildren(item.id, 'orgUsers', 'OrgUser')">{{ item }}</span><el-link type="danger" icon="el-icon-delete" @click="handleDeleteChildren(item.id, 'orgUsers', 'OrgUser', true)">{{ $t('domain.def.delete') }}</el-link></div><el-link type="primary" icon="el-icon-edit" @click="handleCreateChildren('orgUsers', 'OrgUser', 'posts', temp.id, true)">{{ $t('domain.def.add') }}</el-link>
              </el-form-item>
            </el-col>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        ranking: [{ type: 'integer', message: 'ranking must be integer', trigger: 'change' }, { min: 1, max: 99, message: 'ranking must be range(min: 1, max: 99)', trigger: 'change' }],
