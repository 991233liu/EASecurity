    ...
    <div class="card-container search-card">
      <el-card shadow="never">
        <div slot="header" class="card-header">
          <span>{{ $t('domain.uriIp.pageTitle') }}</span>
        </div>
        <el-row :gutter="10" align="middle">

          <el-col :md="8" :sm="12" :xs="24" class="grid-cell">
            <el-form-item :label="'ips'|i18n('uriIp')">
              <el-input v-model="tableQuery.search.ips" type="text" clearable />
            </el-form-item>
          </el-col>

          <el-col :md="8" :sm="12" :xs="24" class="grid-cell">
            <el-form-item :label="'group1'|i18n('uriIp')">
              <el-input v-model="tableQuery.search.group1" type="text" clearable />
            </el-form-item>
          </el-col>

          <el-col :md="8" :sm="12" :xs="24" class="grid-cell">
            <el-form-item :label="'fromTo'|i18n('uriIp')">
              <el-input v-model="tableQuery.search.fromTo" type="text" clearable />
            </el-form-item>
          </el-col>

          <el-col :md="8" :sm="12" :xs="24" class="grid-cell">
            <el-form-item :label="'status'|i18n('uriIp')">
              <el-input v-model="tableQuery.search.status" type="text" clearable />
            </el-form-item>
          </el-col>

          <el-col :md="8" :sm="12" :xs="24" class="grid-cell">
            <el-form-item :label="'uri'|i18n('uriIp')">
    ...
    <div class="card-container list-card">
      <el-card style="{width: 100% !important}" shadow="never">
        ...
        >
          ...
          <el-table-column v-if="config.props.ips.visible" :label="'ips'|i18n('uriIp')" prop="ips" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type" @click="handleShow(row.id)">{{ row.ips }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.group1.visible" :label="'group1'|i18n('uriIp')" prop="group1" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type" @click="handleShow(row.id)">{{ row.group1 }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.fromTo.visible" :label="'fromTo'|i18n('uriIp')" prop="fromTo" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.fromTo | fmatEnum(enumClasses['com.easecurity.core.basis.au.UriIpEnum$FromTo']) }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.status.visible" :label="'status'|i18n('uriIp')" prop="status" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.status | fmatEnum(enumClasses['com.easecurity.core.basis.au.UriIpEnum$Status']) }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.uri.visible" :label="'uri'|i18n('uriIp')" prop="uri" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type span-inline" @click="handleShowChildren(row.uri.id, 'uri', 'Uri')">{{ row.uri }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.id.visible" :label="'id'|i18n('uriIp')" prop="id" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.id }}</span>
            </template>
          </el-table-column>
...
export default {
  name: 'UriIpList', // name值需要与router中的name相同，否则页签的“keep-alive”功能不生效
    ...
    return {
      ...
      config: {
        ...
        props: {
          ...
          ips: {
            title: i18n('ips', 'uriIp'),
            defVisible: true,
            visible: true
          },

          group1: {
            title: i18n('group1', 'uriIp'),
            defVisible: true,
            visible: true
          },

          fromTo: {
            title: i18n('fromTo', 'uriIp'),
            defVisible: true,
            visible: true
          },

          status: {
            title: i18n('status', 'uriIp'),
            defVisible: true,
            visible: true
          },

          uri: {
            title: i18n('uri', 'uriIp'),
            defVisible: true,
            visible: true
          },
        ...
        // 导出Excel配置
        download: {
          excelName: this.$t('domain.uriIp.pageTitle'),
      ...
      enumClasses: {
        ...
        'com.easecurity.core.basis.au.UriIpEnum$FromTo': this.getEnum('com.easecurity.core.basis.au.UriIpEnum$FromTo'),

        'com.easecurity.core.basis.au.UriIpEnum$Status': this.getEnum('com.easecurity.core.basis.au.UriIpEnum$Status')
