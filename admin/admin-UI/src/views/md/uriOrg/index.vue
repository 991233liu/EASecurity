    ...
    <div class="card-container search-card">
      <el-card shadow="never">
        <div slot="header" class="card-header">
          <span>{{ $t('domain.uriOrg.pageTitle') }}</span>
        </div>
    ...
    <div class="card-container list-card">
      <el-card style="{width: 100% !important}" shadow="never">
        ...
        >
          ...
          <el-table-column v-if="config.props.orgId.visible" :label="'orgId'|i18n('uriOrg')" prop="orgId" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type" @click="handleShow(row.id)">{{ row.orgId }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.annotation.visible" :label="'annotation'|i18n('uriOrg')" prop="annotation" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type" @click="handleShow(row.id)">{{ row.annotation }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.group1.visible" :label="'group1'|i18n('uriOrg')" prop="group1" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.group1 }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.fromTo.visible" :label="'fromTo'|i18n('uriOrg')" prop="fromTo" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.fromTo | fmatEnum(enumClasses['com.easecurity.core.basis.au.UriOrgEnum$FromTo']) }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.status.visible" :label="'status'|i18n('uriOrg')" prop="status" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.status | fmatEnum(enumClasses['com.easecurity.core.basis.au.UriOrgEnum$Status']) }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.uri.visible" :label="'uri'|i18n('uriOrg')" prop="uri" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span class="link-type span-inline" @click="handleShowChildren(row.uri.id, 'uri', 'Uri')">{{ row.uri }}</span>
            </template>
          </el-table-column>

          <el-table-column v-if="config.props.id.visible" :label="'id'|i18n('uriOrg')" prop="id" sortable="custom" min-width="100px" align="center">
            <template slot-scope="{row}">
              <span>{{ row.id }}</span>
            </template>
          </el-table-column>
...
export default {
  name: 'UriOrgList', // name值需要与router中的name相同，否则页签的“keep-alive”功能不生效
    ...
    return {
      ...
      config: {
        ...
        props: {
          ...
          orgId: {
            title: i18n('orgId', 'uriOrg'),
            defVisible: true,
            visible: true
          },

          annotation: {
            title: i18n('annotation', 'uriOrg'),
            defVisible: true,
            visible: true
          },

          group1: {
            title: i18n('group1', 'uriOrg'),
            defVisible: true,
            visible: true
          },

          fromTo: {
            title: i18n('fromTo', 'uriOrg'),
            defVisible: true,
            visible: true
          },

          status: {
            title: i18n('status', 'uriOrg'),
            defVisible: true,
            visible: true
          },

          uri: {
            title: i18n('uri', 'uriOrg'),
            defVisible: true,
            visible: true
          },

          id: {
            title: i18n('id', 'uriOrg'),
            defVisible: false
          }
        ...
        // 导出Excel配置
        download: {
          excelName: this.$t('domain.uriOrg.pageTitle'),
