...
//    @Secured(["ROLE_USER"])
    def index(Integer max) {
        ...
        Map searchParams = [:]
        if (params.search) {
            searchParams = ServletUtils.getSearchParams(params.search, RoleGroup)
        }
        String where = searchParams.remove('where')
        if (searchParams) {
            String orderBy = params.sort ? " order by ${params.sort} ${params.order}" : ""
            int roleGroupCount = RoleGroup.executeQuery("select count(id) from RoleGroup where" + where, searchParams)[0]
            List roleGroupList = RoleGroup.findAll("from RoleGroup where" + where + orderBy, searchParams, params)
            respond roleGroupList, view: "${VIEW_PATH}index", model: [roleGroupCount: roleGroupCount]
        } else {
