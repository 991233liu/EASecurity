...
//    @Secured(["ROLE_USER"])
    def index(Integer max) {
        ...
        Map searchParams = [:]
        if (params.search) {
            searchParams = ServletUtils.getSearchParams(params.search, Org)
        }
        String where = searchParams.remove('where')
        if (searchParams) {
            String orderBy = params.sort ? " order by ${params.sort} ${params.order}" : ""
            int orgCount = Org.executeQuery("select count(id) from Org where" + where, searchParams)[0]
            List orgList = Org.findAll("from Org where" + where + orderBy, searchParams, params)
            respond orgList, view: "${VIEW_PATH}index", model: [orgCount: orgCount]
        } else {
