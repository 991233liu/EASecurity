...
//    @Secured(["ROLE_USER"])
    def index(Integer max) {
        ...
        Map searchParams = [:]
        if (params.search) {
            searchParams = ServletUtils.getSearchParams(params.search, Posts)
        }
        String where = searchParams.remove('where')
        if (searchParams) {
            String orderBy = params.sort ? " order by ${params.sort} ${params.order}" : ""
            int postsCount = Posts.executeQuery("select count(id) from Posts where" + where, searchParams)[0]
            List postsList = Posts.findAll("from Posts where" + where + orderBy, searchParams, params)
            respond postsList, view: "${VIEW_PATH}index", model: [postsCount: postsCount]
        } else {
